...
| Section | Purpose |
|---|---|
| State + Constants | `state` object (incl. `state.stepConfigs` (monthly steps), `state.dayStepConfigs` (day-view lines, sub-step shape `{name, ot, br, hpd}`, threshold = `hpd ?? (hrs/shift + ot + br) × shifts`) + `state.weekStepConfigs` (week-view lines, **combined month+day shape** `{wd, otDays, otNormal, hdWorked, otHoliday, br, hpd}`, threshold = `hpd·wd` else `[wd·(hrs+br) + otDays·otNormal + hd·((hrs+br)+otHoliday)]·shifts`). `activeStepConfigs()` picks month/week/day; `isSubStep()` = has `ot` & no `wd` (day only); `computeStepThreshold` handles all three (month has br=0/hpd=∅ ⇒ unchanged); `getStepDefs()` drops Max Cap in **day** view only (`periodHasMaxCap()`)), `state.maxCapOT`), `PROCESSES`, `LINE_NAMES`, `DEFAULT_STEP_CONFIGS`, `DEFAULT_SUBSTEP_CONFIGS`, `STEP_LINE_STYLES`, `DATASET_COLORS`, `MAX_CHAIN_DEPTH` |
...
| Chart helpers | `xHierarchyPlugin`, `computeXHierarchy()` (+`xGroupLabel()`), `shouldUseNumericMonths()`, `getMonthOnly()` |
| Time granularity | `state.granularity` (`'month'`/`'week'`/`'day'`). A **period key** is the atomic time bucket (`Apr'26` / `2026-W15` / `2026-04-15`); the whole calc/render pipeline is period-key agnostic. `periodType()`, `isoWeekKey()`/`isoWeekStart()`, `keyStartDate()`, `periodCalDays()` (month days / 7 / 1), `periodWorkDays()` (Mon–Fri, for sub-month threshold scaling), `rebucketDataset(ds,gran)` re-rolls `monthlyData.daily` (per-part per-ISO-date, kept by `parseRawData` when the source has real dates via `rawCellDate()`) into `monthlyData.months`+`.data`. `monthOrdinal()` is date-based (days-since-epoch) so month/week/day keys are mutually comparable — FY still sorts last. `calculateMaxCap`/`computeStepThresholdMonth` branch on `periodType`. `setGranularity()` recomputes all steps; `syncGranularityControl()` disables week/day when no dataset has `daily`. Persisted (localStorage + snapshot). **Month view is byte-identical to pre-3.12.** Phase-1 limits: FY columns are month-view-only; the 📅 per-month editor + month-relabel range stay month-based. `state.weekNumbering` (`'iso'`/`'month'`, persisted) is a **week-view label style only**: `weekLeafLabel()`/`weekAnchorDate()` (both keyed off it) drive `getMonthOnly()`+`xGroupLabel()` — `'month'` assigns a week to its **end (Sunday)** month and numbers `ceil(sundayDay/7)` (W1..W5/6, reset per month) instead of the ISO week number. `isoWeekEnd()` helper; UI = `#weekNumbering` in `#weekNumWrap`, shown only in week view by `syncGranularityControl()` |
...
| Time Level (v4) | **Temporal load-leveling overlay** — moves a part's pieces between PERIODS on the **same line** within a container (its ISO week, or its **end-Sunday month**), pulling each period toward the container's **mean** hours so the bars come out ~equal (not capped at a step). `timeLevelBalance(byProcess, partMap, setting, months, container, direction, targetIdx)` (direction `'back'`=build-ahead/earlier · `'fwd'`=delay/later · `'both'`); `periodContainerKey()` groups periods; `cloneByProcess()` deep-clones so it never mutates the ladder. It's a per-dataset **render overlay**: `state.datasets[].timeLevel = {on,container,direction}` (persisted localStorage + snapshot); `getCurrentResult()` returns `deriveTimeLevelResult()` (sig-cached on `ds._tlCache`, `settingsSig()`) when `timeLevel.on`. Master is present in snapshots so it re-derives read-only there. Cleared/invalidated in `setGranularity` when the container is invalid for the new period. UI = `#balanceRow` toolbar (`#lineBalTarget`+`#lineBalanceBtn` for Line Balance via `gotoStep()`; `#tlContainer`/`#tlDirection`/`#timeLevelBtn`/`#timeLevelOffBtn` gated by `syncGranularityControl`; `syncBalanceToolbar()` syncs it). **Submit** = `#calculateBtn` (relabeled, step-0 init). The Smart-Balance ladder + step chips are unchanged underneath |
| Rendering | `renderPreview()`, `renderCalendarGrid()`, `renderTableArea()`, `renderChart()`, `renderResults()`, etc. |
| Y-axis unit | `state.yUnit` (`'hours'`/`'pcs'`) toggles the Results chart + table between hours and pieces. Helpers: `getYUnit()`, `allocBarVal()`, `lineBarVal()`, `lineQtyTotal()`, `lineHrsPerPiece()` (per-line per-month **actual**-mix hrs/piece — used for table cells), `thPieces()` (hours→pieces via a supplied factor), `fmtY()`/`fmtCellY()`, `yAxisTitle()`. **Threshold** lines (steps + Max Cap) convert to pieces with a **fixed product mix** (each part weighted by its total pieces over the shown months) but **each month's own CT** — `lineHrsPerPieceStable()` (byPart, per line) / `procHrsPerPieceStable()` (byLine, process-wide), built on `lineMixWeights()`/`mixHpp()`/`getPartOA()`/`getBaseCT()`. So a threshold line stays **straight** while CT is stable and **steps only where CT changes** (e.g. an override `From FY28`), instead of wiggling with monthly demand. (History: pre-v3.7.2 used an all-months avg → flat but hid CT steps; v3.7.2–v3.10.1 used per-month actual mix → stepped but wiggled; v3.11 fixed-mix gets both. Bars still use the real monthly mix; cell colour is by the hours ratio, so over/under stays truthful.) Tables show pieces; Diff table stays in hours |
