...
| Y-axis unit | `state.yUnit` (`'hours'`/`'pcs'`) toggles the Results chart + table between hours and pieces. Helpers: `getYUnit()`, `allocBarVal()`, `lineBarVal()`, `lineQtyTotal()`, `procHrsPerPiece()`/`procHrsPerPieceAvg()` (process-wide, used only by byLine), `lineHrsPerPiece()`/`lineHrsPerPieceAvg()` (**per-line** hrs/piece — byPart threshold conversion, so pcs over/under matches hours), `thPieces()` (hours→pieces via a supplied factor), `fmtY()`/`fmtCellY()`, `yAxisTitle()`. byPart converts each threshold **per line** (non-variant → line's all-months avg = flat within the group; variant/Max Cap → per month). Tables show pieces (colour still by hours ratio). byLine uses the process-wide avg; Diff table stays in hours |
