...
| Section | Purpose |
|---|---|
| State + Constants | `state` object (incl. `state.stepConfigs` (monthly steps), `state.dayStepConfigs` + `state.weekStepConfigs` (independent day- & week-view capacity lines — `{name, ot, br, hpd}` + week adds `days`; per-working-day threshold = `hpd ?? (hrs/shift + ot + br) × shifts`, week × `days`; `activeStepConfigs()` picks by granularity, `isSubStep()`; `getStepDefs()` drops Max Cap in **day** view (`periodHasMaxCap()`)), `state.maxCapOT`), `PROCESSES`, `LINE_NAMES`, `DEFAULT_STEP_CONFIGS`, `DEFAULT_SUBSTEP_CONFIGS`, `STEP_LINE_STYLES`, `DATASET_COLORS`, `MAX_CHAIN_DEPTH` |
