...
- **Time-Level balancing** — in Week/Day view, level the load across *periods on the same line* within a container (Day → within Week/Month, Week → within Month) so each period comes out ~equal (pulled toward the container average), choosing direction (build ahead / delay / both). Volume is redistributed in time, never created or destroyed
